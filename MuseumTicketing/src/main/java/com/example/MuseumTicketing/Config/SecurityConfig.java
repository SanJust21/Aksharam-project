    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(request -> request
                        ...
                        .requestMatchers("/deleteMain/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/deleteByFirstSub/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/deleteSecond/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/DataEntry2/firstSub").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/DataEntry2/genCommonId").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/imgData/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/dataType1/addtalk").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/DataEntry1/mainT").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/mediaData/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/fileType/addFiletype").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/qrcode/generate").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/DataEntry3/secondSub").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/DataEntry3/generateSSid").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/updateFirst/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/updateMain/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
                        .requestMatchers("/updateSecondSub/**").hasAnyAuthority(Role.ADMIN.name(),Role.EMPLOYEE.name())
